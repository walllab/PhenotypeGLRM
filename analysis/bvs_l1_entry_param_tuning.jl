...
# construct the model
rx, ry = NonNegOneReg(10^(reg-2)), QuadReg(.01)
...
# write to file
writecsv(string(data_directory, "/impute_bvs_simplex_cv_train_error$(k)_$(10^(reg-2)).csv"), train_error)
writecsv(string(data_directory, "/impute_bvs_simplex_cv_test_error$(k)_$(10^(reg-2)).csv"), test_error)

# fit full model
#@time X,Y,ch = fit!(glrm, ProxGradParams(max_iter=500));

# write to file
#writecsv(string(data_directory, "/impute_bvs_simplex_initk_X$(k)_$(10^(reg-2)).csv"), X)
#writecsv(string(data_directory, "/impute_bvs_simplex_initk_Y$(k)_$(10^(reg-2)).csv"), Y)
#writecsv(string(data_directory, "/impute_bvs_simplex_initk_Z$(k)_$(10^(reg-2)).csv"), impute(glrm))
