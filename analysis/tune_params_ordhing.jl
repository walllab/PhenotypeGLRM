...
function run_model(fold, k)
	...
	@time writecsv(string(data_directory, "/impute_ordhing_X_cv_k$(k)_fold$(fold).csv"), X)
	@time writecsv(string(data_directory, "/impute_ordhing_Y_cv_k$(k)_fold$(fold).csv"), Y)
