...
with open(filename + '.csv', 'r') as infile:
	...
	label_cols = [
		...
		header.index('ADOS_Module1:diagnosis'), header.index('ADOS_Module1:diagnosis_num_nulls'), header.index('ADOS_Module1:communication'), header.index('ADOS_Module1:social_interaction'), header.index('ADOS_Module1:restricted_repetitive_behavior'),
		header.index('ADOS_Module2:diagnosis'), header.index('ADOS_Module2:diagnosis_num_nulls'), header.index('ADOS_Module2:communication'), header.index('ADOS_Module2:social_interaction'), header.index('ADOS_Module2:restricted_repetitive_behavior'),
		header.index('ADOS_Module3:diagnosis'), header.index('ADOS_Module3:diagnosis_num_nulls'), header.index('ADOS_Module3:communication'), header.index('ADOS_Module3:social_interaction'), header.index('ADOS_Module3:restricted_repetitive_behavior'),
		header.index('ADOS_Module4:diagnosis'), header.index('ADOS_Module4:diagnosis_num_nulls'), header.index('ADOS_Module4:communication'), header.index('ADOS_Module4:social_interaction'), header.index('ADOS_Module4:restricted_repetitive_behavior'),
			...
			if ':' in h:
				...
				for piece in pieces:
					if piece in json_item:
						...
						print('Problem!', piece)
				if 'data-type' in json_item and json_item['data-type'] == 'ordinal' and not pieces[0] == 'ADOS':
					keep_cols.append(i)

	print('Keeping %d features' % (len(keep_cols)-1))

	with open(filename + '_filtered2_labels.csv', 'w+') as label_outfile:
		with open(filename + '_filtered2.csv', 'w+') as outfile:
